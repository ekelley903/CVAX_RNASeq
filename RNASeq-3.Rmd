...
sigGenes <- as.data.frame(cvax_results_8v0) %>%
  filter(padj < 0.05, !is.na(ensembl)) %>%
  pull(ensembl)
...
sigGenesSym <- as.data.frame(cvax_results_8v0) %>%
  filter(padj < 0.05, !is.na(hgnc_symbol)) %>%
  pull(hgnc_symbol)
...
bp <- ggplot(arrange(egmtvax@result[1:20, ], Count), aes(GeneRatio_num, reorder(ID, Count), color = p.adjust, size = Count)) +
  geom_point() +
  scale_color_viridis_c(option = "plasma") +
  ylab("MSigDB C7 VAX Signature") +
  xlab("Gene Ratio") +
  ggtitle("Day 8: Vaccine Response Gene Sets") +
  theme_bw()
bp
...
ClusterProfiler with Immport genes as the database.
```{r}
...
plot_Immport <- ggplot(arrange(ImmportCP@result[1:6, ], Count), aes(GeneRatio_num, reorder(ID, Count), size = Count, color = p.adjust < 0.05)) +
  geom_point() +
  scale_color_viridis_c(option = "plasma") +
  ylab("Immport Category") +
  xlab("Gene Ratio") +
  ggtitle("Day 8: Enriched Gene Sets Immport") +
  theme_bw()
plot_Immport
```

GSEA using ClusterProfiler. The gene list must be ordered by L2FC first.
```{r}
...
ig <- res_8v0_df %>%
  filter(str_detect(hgnc_symbol, "IG")) %>%
